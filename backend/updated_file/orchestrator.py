...
# --------------------------------------------------
    ...
    if intent == "identity":
        ...
        log_event(
            ...
            request_id=request_id,
            route="deterministic",
            reason="identity_shortcut"
        )

        response = (
        "I’m Bro-Buddy — a grounded AI companion designed to help you think clearly.\n\n"
        "You can switch between:\n"
        "• Professional mode → structured guidance for work and decisions\n"
        "• AI mode → deeper technical and system-level insights"
        )
    ...
    if intent in ("professional_reasoning", "ai_knowledge"):
        ...
        log_event(
            "llm_raw_response",
            request_id=request_id,
            preview=model_answer[:200],
            output_word_estimate=len(model_answer.split())
        )
        # ==================================================
