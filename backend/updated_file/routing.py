...
def classify_intent(user_query: str, mode: str) -> str:
    ...
    # 0. Identity Shortcut
    for phrase in IDENTITY_PATTERNS_SIMPLE:
        if phrase in text:
            return "identity"
    
    # 1️. Greeting (Highest Priority)
    if _is_greeting(text):
        return "greeting"

    # 2️.  AI Context Pair (Strongest Signal)
    if ai_pair_flag:
        return "ai_knowledge"

    # 3️. AI Keyword Detection
    if ai_keyword_flag:
        return "ai_knowledge"
